...
<body>
    ...
    <div class="main">
        ...
        <div id="plates" class="tab-content">
            ...
            <table>
                <thead><tr><th>Номер</th><th>Владелец</th><th>Цена</th><th>Редкость</th><th>Статус</th><th>Действия</th></tr></thead>
                <tbody id="plates-table-body"><tr><td colspan="6" style="text-align: center;">Загрузка...</td></tr></tbody>
            </table>
        </div>
        <div id="users" class="tab-content">
            <h2>Реестр пользователей</h2>
            <table>
                <thead><tr><th>Пользователь / ID</th><th>Количество номеров</th><th>Количество штрафов</th><th>Общая стоимость номеров</th></tr></thead>
                <tbody id="users-table-body"><tr><td colspan="4" style="text-align: center;">Загрузка...</td></tr></tbody>
            </table>
        </div>
        <div id="fines" class="tab-content">
            ...
            <table>
                <thead><tr><th>Статья</th><th>Пользователь</th><th>Дата</th><th>Причина</th><th>Действия</th></tr></thead>
                <tbody id="fines-table-body"><tr><td colspan="5" style="text-align: center;">Загрузка...</td></tr></tbody>
            </table>
        </div>
        <div id="auction" class="tab-content">
            <h2>Аукцион госномеров</h2>
            <div id="auction-container" style="background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 30px; text-align: center; max-width: 500px; margin: 40px auto;">Загрузка аукциона...</div>
        </div>
        <div id="dossier" class="tab-content">
            ...
            <div id="dossier-result" style="margin-top: 20px;"><p style="color: #8b949e;">Введите номер выше для получения полной оперативной сводки.</p></div>
        </div>
    </div>
    ...
    <script>
            ...
            tbody.innerHTML = data.map(p => `
                <tr>
                    ...
                    <td>
                        ...
                        ${p.wanted ? `<button class="btn btn-success" title="Снять с розыска" onclick="unwantedPlate('${p.plate}')">✅</button>` : `<button class="btn btn-danger" title="Объявить в розыск" onclick="openWantedModal('${p.plate}')">🚨</button>`}
                    </td>
                </tr>
            `).join('');
        }
        ...
        function addFine() {
            ...
            if(!code || !user || !reason) { showToast('Заполните все поля штрафа', 'error'); return; }
            fetch('/api/fine/add', {
                method: 'POST',
        ...
        function removeFine(code, userId) {
            ...
            fetch('/api/fine/remove', {
                ...
                if(resp.success) { showToast('Штраф снят'); loadFines(); } else { showToast('Ошибка', 'error'); }
            });
        }
        function loadAuction() {
            fetch('/api/auction').then(res => res.json()).then(data => {
                const container = document.getElementById('auction-container');
                if(!data.plate) { container.innerHTML = '<p style="color: #8b949e;">❌ На аукционе нет активных лотов</p>'; return; }
                container.innerHTML = `
        ...
        function placeBid() {
            ...
            if(!amount) { showToast('Введите корректную сумму', 'error'); return; }
            fetch('/api/bid', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({amount})
            }).then(res => res.json()).then(resp => {
                if(resp.success) { showToast('Ставка принята!'); loadAuction(); } else { showToast(resp.error || 'Ставка слишком мала', 'error'); }
            });
        }
        function endAuction() {
            if(!confirm('Завершить аукцион?')) return;
            fetch('/api/auction/end', {method: 'POST'}).then(() => { showToast('Аукцион завершен'); loadAuction(); });
        }
        ...
        function loadDossier() {
            ...
            fetch(`/api/dossier/${encodeURIComponent(plate)}`).then(res => res.json()).then(data => {
                ...
                if(!data.found) { resDiv.innerHTML = `<p style="color: #da3633;">Транспортное средство с номером ${plate} не найдено в базе.</p>`; return; }
                resDiv.innerHTML = `
                    <div style="background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 20px;">
                        ...
                        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                            ...
                            ${data.wanted ? `<button class="btn btn-success" onclick="unwantedPlate('${data.plate}'); setTimeout(loadDossier, 300);">✅ Снять розыск</button>` : `<button class="btn btn-danger" onclick="openWantedModal('${data.plate}')">🚨 Розыск</button>`}
                        ...
                        <table>
                            ...
                            <tbody>${data.history.length === 0 ? '<tr><td colspan="3">История пуста</td></tr>' : data.history.map(h => `<tr><td>${h.date}</td><td><b>${h.type}</b></td><td>${h.desc}</td></tr>`).join('')}</tbody>
                        </table>
                    </div>
                `;
            });
        ...
        function submitGivePlate() {
            ...
            if(!plate) { showToast('Введите госномер', 'error'); return; }
            fetch('/api/give', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({user_id, plate})
            }).then(res => res.json()).then(resp => {
                if(resp.success) { showToast('Номер успешно выдан'); closeModal('modal-give'); loadPlates(); } else { showToast(resp.error || 'Номер уже занят', 'error'); }
            });
        }
        function revokePlate(plate) {
            ...
            fetch('/api/revoke', {
                ...
                if(resp.success) { showToast('Номер успешно изъят'); loadPlates(); } else { showToast('Ошибка изъятия', 'error'); }
            });
        }
        ...
        function submitProtocol() {
            ...
            if(!desc) { showToast('Заполните описание', 'error'); return; }
            fetch('/api/protocol', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({plate, type, desc})
            }).then(res => res.json()).then(resp => {
                if(resp.success) { showToast('Протокол успешно составлен'); closeModal('modal-protocol'); loadPlates(); } else { showToast('Ошибка', 'error'); }
            });
        }
        ...
        function submitWanted() {
            ...
            if(!reason) { showToast('Укажите причину розыска', 'error'); return; }
            fetch('/api/wanted', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({plate, reason})
            }).then(res => res.json()).then(resp => {
                if(resp.success) { showToast('Транспорт объявлен в розыск'); closeModal('modal-wanted'); loadPlates(); } else { showToast('Ошибка', 'error'); }
            });
        }
        function unwantedPlate(plate) {
            ...
            fetch('/api/unwanted', {
                ...
                if(resp.success) { showToast('Розыск успешно снят'); loadPlates(); } else { showToast('Ошибка', 'error'); }
            });
        }
        setInterval(() => {
